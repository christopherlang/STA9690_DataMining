...
Knowing that predictor matrix $X\in\mathbb{R}$ is an orthogonal matrix:

$$||y-X\beta||^2_2=y^Ty-2y^TX\beta+\beta^TX^TX\beta=y^Ty-2y^TX\beta+\beta^TI\beta$$
$$y^Ty-2y^TX\beta+\beta^T\beta=Xy^TX^Ty-2y^TX^{-1}X\beta+\beta^T\beta$$
\newpage
...
When looking at just the lasso images, the main difference between the usual rule and standard error rule is that the usual rule is slightly more noisy than standard error, but more saturated in coloration (specifically the darker areas). Which image is found to look better is a bit subjective, but we found that the usual rule image is better. Though it is more noisy (though only slighty), it seems closer to the original `bstar` image due to the closer coloration it has

Between the ridge and lasso images, the difference is much more stark however. The ridge images is simply far too noisy, which causes it to lose details in the shape, as well as reducing the color's saturation compared to the lasso iamges

Ultimately, the lasso images are much closer to the original `bstar` image than ridge, primarily due to the closer color match it has to the original. As stated before, of the four images, the lasso usual rule image is found to be the closests to the original `bstar` image
